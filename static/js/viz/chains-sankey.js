/**
 * Chainsmith Viz — Chains Sankey (findings → attack chains graph).
...
(function (ns) {
     ...
     */
    ns.renderChainsList = function (chains, findings, openModal) {
        ...
        list.innerHTML = chains.map(function (chain) {
            return '<div class="chain-card source-' + chain.source + '" data-chain="' + chain.id + '">' +
                ...
                '<div class="chain-card-findings">' +
                chain.finding_ids.map(function (id) { return '<span class="chain-finding-tag">' + id + '</span>'; }).join('') +
                '</div></div>';
        }).join('');
        ...
        list.querySelectorAll('.chain-card').forEach(function (card) {
            card.addEventListener('click', function () {
                ...
                if (chain) openModal(chain.title, getChainModalContent(chain, findings));
            });
        });
    };
     ...
     */
    ns.renderChainsGraph = function (chains, findings, openModal) {
        ...
        var usedFindings = new Set();
        chains.forEach(function (chain) {
            chain.finding_ids.forEach(function (fid) { usedFindings.add(fid); });
        });

        var findingIndices = new Map();
        usedFindings.forEach(function (fid) {
            var finding = findings.find(function (f) { return f.id === fid; });
            findingIndices.set(fid, nodes.length);
            nodes.push({
                name: finding ? fid + ': ' + finding.title : fid,
                type: 'finding',
                severity: finding ? finding.severity : 'info',
                data: finding,
            });
        });
        ...
        chains.forEach(function (chain) {
            ...
            chain.finding_ids.forEach(function (fid) {
                var sourceIdx = findingIndices.get(fid);
        ...
        function highlightConnectedPaths(findingNode, highlight) {
            var connectedChainIds   = new Set();
            var connectedFindingIds = new Set();

            graph.links.forEach(function (l) {
                if (l.source === findingNode) connectedChainIds.add(l.target.index);
            });
            graph.links.forEach(function (l) {
                if (connectedChainIds.has(l.target.index)) connectedFindingIds.add(l.source.index);
            });
            ...
            node.select('rect').attr('opacity', function (d) {
                ...
                if (d === findingNode) return 1;
                if (connectedChainIds.has(d.index)) return 1;
                if (connectedFindingIds.has(d.index)) return 0.9;
                return 0.2;
            });
            node.select('text').attr('opacity', function (d) {
                if (!highlight) return 1;
                if (d === findingNode || connectedChainIds.has(d.index) || connectedFindingIds.has(d.index)) return 1;
                return 0.3;
            });
        }
        ...
        function highlightChainPaths(chainNode, highlight) {
            var connectedFindingIds = new Set();
            graph.links.forEach(function (l) {
                if (l.target === chainNode) connectedFindingIds.add(l.source.index);
            });
            ...
            node.select('rect').attr('opacity', function (d) {
                if (!highlight) return 0.9;
                if (d === chainNode || connectedFindingIds.has(d.index)) return 1;
                return 0.2;
            });
            node.select('text').attr('opacity', function (d) {
                if (!highlight) return 1;
        ...
        var node = svg.append('g')
            ...
            .on('mouseenter', function (event, d) {
                if (d.type === 'finding') highlightConnectedPaths(d, true);
                else if (d.type === 'chain') highlightChainPaths(d, true);
            })
            .on('mouseleave', function (event, d) {
                if (d.type === 'finding') highlightConnectedPaths(d, false);
                else if (d.type === 'chain') highlightChainPaths(d, false);
            })
            .on('click', function (event, d) {
                if (d.type === 'chain') openModal(d.data.title, getChainModalContent(d.data, findings));
                else if (d.data) openModal(d.data.title, getFindingModalContent(d.data));
            });
        ...
        node.append('text')
            .attr('x', function (d) { return d.type === 'finding' ? (d.x1 - d.x0) + 6 : -6; })
            .attr('y', function (d) { return (d.y1 - d.y0) / 2; })
            .attr('dy', '0.35em')
            .attr('text-anchor', function (d) { return d.type === 'finding' ? 'start' : 'end'; })
            .attr('fill', 'var(--text-primary)')
            .attr('font-size', '11px')
            .text(function (d) {
                var maxLen = d.type === 'finding' ? 35 : 30;
                var name = d.name || '';
