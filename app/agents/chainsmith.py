...
from app.config import (
    LITELLM_BASE_URL, LITELLM_MODEL_CHAINSMITH,
    ATTACK_PATTERNS_PATH
)
from app.models import (
    Finding, FindingStatus, AttackChain, ScopeDefinition,
    AgentType, AgentEvent, EventType, EventImportance
)
...
What is your target domain?"""
    ...
    async def continue_scoping(self, user_message: str) -> dict:
        ...
        wants_to_go = any(word in msg_lower for word in ["go", "launch", "start", "proceed", "done", "ready"])
        
        # Check if user wants defaults
        wants_defaults = any(phrase in msg_lower for phrase in ["default", "use default", "skip", "none"])
        
        # If user wants to go or use defaults, check what we have
        if wants_to_go:
            if not self.target:
                return self._respond("I still need a target domain before we can proceed. What domain should I test?")
    ...
    def _extract_scope_info(self, msg: str):
        ...
        for line in msg.split('\n'):
            ...
            domains = re.findall(domain_pattern, line, re.IGNORECASE)
            # Get full matches
            full_domains = re.findall(r'(\*\.[a-zA-Z0-9][-a-zA-Z0-9.]*|[a-zA-Z0-9][-a-zA-Z0-9]*\.[a-zA-Z0-9][-a-zA-Z0-9.]*)', line)
            
            # Determine context
            is_exclusion = any(w in line_lower for w in ["exclude", "skip", "avoid", "except", "out of scope", "don't test", "not "])
            is_target = any(w in line_lower for w in ["target", "scope", "test", "scan"]) or (not is_exclusion and not self.target)
            ...
            # Check for timeframe
            time_indicators = ["hour", "time", "until", "before", "after", "restrict", "window", "anytime", "no restrict", "24/7"]
    ...
    def _build_response(self) -> dict:
        ...
        if not self.scope_status["targets"]:
            ...
            prompt = "Any timeframe restrictions? (e.g., 'business hours only' or 'no restrictions')"
        else:
    ...
    def _finalize_scope(self) -> dict:
        ...
        self.scope = ScopeDefinition(
            ...
            allowed_techniques=[
                "port_scan", "header_grab", "robots_fetch",
                "directory_enum", "chatbot_probe", "error_trigger"
            ],
            forbidden_techniques=[
                "dos", "data_exfiltration", "credential_stuffing",
