...
class ChainOrchestrator:
    ...
    def __init__(
        ...
        self.current_phase: ExecutionPhase | None = None
        ...
        for suite in SUITE_ORDER:
            ...
            while suite_pending:
                ...
                for node in suite_pending:
                    ...
                    deps_satisfied = all(
                        dep in executed or dep not in self.nodes for dep in node.dependencies
                    )
    ...
    async def run(self, initial_context: dict[str, Any] = None) -> list[Observation]:
        ...
        for phase in phases:
            ...
            if phase.parallel:
                await asyncio.gather(*[self._run_check(check) for check in phase.checks])
            else:
    ...
    async def _run_check(self, check: BaseCheck):
        ...
        for key, value in result.outputs.items():
            ...
            if (
                key in self.context
                and isinstance(self.context[key], list)
    ...
    def _is_in_scope(self, url: str) -> bool:
        ...
        try:
            ...
            # Check scope
            return any(host_matches_pattern(host, scope) for scope in self.scope_domains)
        except Exception:
            return False
