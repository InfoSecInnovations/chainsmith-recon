...
class DefaultCredsCheck(ServiceIteratingCheck):
    ...
    reason = "Admin panels with default credentials supersede all AI-specific findings on the same host"
    ...
    ADMIN_PATTERNS = [
        ...
        "/dashboard", "/manager", "/wp-admin", "/panel",
    ]
    ...
    async def check_service(self, service: Service, context: dict[str, Any]) -> CheckResult:
        ...
        admin_paths = [p for p in accessible if any(a in p.lower() for a in ["/admin", "/console", "/dashboard", "/manager", "/panel"])]
    ...
    async def _try_credentials(
        self, client: AsyncHttpClient, service: Service, path: str, form_html: str,
    ) -> tuple[str, str] | None:
        ...
        for username, password in self.DEFAULT_CREDS:
            ...
            if resp.status_code in (301, 302, 303) and not any(p.search(body) for p in self.LOGIN_FAILURE_PATTERNS):
                return (username, password)
    ...
    def _looks_like_admin_content(body: str) -> bool:
        ...
        admin_indicators = ["dashboard", "admin panel", "configuration", "settings", "manage", "users"]
