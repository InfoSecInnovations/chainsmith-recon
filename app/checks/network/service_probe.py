...
class ServiceProbeCheck(ServiceIteratingCheck):
    ...
    AI_HEADERS = [
        "x-model", "x-inference", "x-llm", "x-ai-", "x-ml-"
    ]
    
    AI_POWERED_BY = [
        "vllm", "ollama", "tgi", "triton", "inference", "mlflow",
        "huggingface", "transformers", "langchain", "langserve"
    ]
    ...
    async def check_service(self, service: Service, context: dict[str, Any]) -> CheckResult:
        ...
        for scheme in ["https", "http"]:
            ...
            try:
                async with AsyncHttpClient(HttpConfig(timeout_seconds=10.0, verify_ssl=False)) as client:
    ...
    def _classify_service(self, headers: dict, content_type: str, body: str) -> str:
        ...
        for header in self.AI_HEADERS:
            if any(header in k for k in headers_lower.keys()):
                return "ai"
        ...
        ai_body_indicators = [
            ...
            "chatbot", "llm", "inference", "model", "/api/generate"
        ]
    ...
    def _analyze_response(self, service: Service, headers: dict, response) -> list:
        ...
        standard_headers = {
            ...
            "x-xss-protection", "x-request-id", "x-correlation-id",
            "x-cache", "x-cache-hits", "x-served-by"
        }
        ...
        for key, value in headers.items():
            ...
            if key_lower.startswith("x-") and key_lower not in standard_headers:
                severity = "info"
                if any(s in key_lower for s in ["model", "token", "key", "secret", "internal", "debug"]):
                    severity = "medium"
