...
from app.proof_of_scope import ScopeStatus, traffic_logger
...
async def run_scan(state: "AppState"):
    ...
    try:
        ...
        logger.info(
            f"get_all_checks() returned {len(real_checks)} checks: {[c.name for c in real_checks]}"
        )
        ...
        if mgr.is_active:
            ...
            logger.info(
                f"Scenario '{mgr.active.name}' active — {len(simulations)} simulated checks available"
            )
...
async def run_checks_with_progress(runner: CheckRunner, state: "AppState"):
    ...
    while iteration < max_iterations:
        ...
        for c in runner.checks:
            ...
            logger.info(
                f"  {c.name}: status={c.status.value}, pending={is_pending}, can_run={can_run}, missing={missing}"
            )

        # Find runnable checks (pending + conditions satisfied)
        runnable = [c for c in runner.checks if c.status.value == "pending" and c.can_run(context)]
        ...
        for check in runnable:
            ...
            try:
                ...
                logger.info(
                    f"{check.name} completed. success={result.success}, errors={result.errors}"
                )
                logger.info(f"{check.name} outputs: {list(result.outputs.keys())}")
                logger.info(
                    f"{check.name} findings: {len(result.findings)}, services: {len(result.services)}"
                )
            except Exception as e:
            ...
            logger.info(
                f"After {check.name}, context['target_hosts'] = {context.get('target_hosts', 'NOT SET')!r}"
            )
...
def _log_check_traffic(check_name: str, context: dict, result, state: "AppState"):
    ...
    for svc in services:
        ...
        if state.scope_checker:
            host = (
                svc_url.split("://")[1].split("/")[0].split(":")[0] if "://" in svc_url else svc_url
            )
