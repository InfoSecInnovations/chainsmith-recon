...
class TrafficEntryType(str, Enum):
...
class EngagementWindow(BaseModel):
    ...
    def is_within_window(self) -> bool:
        ...
        if self.start:
            start_dt = datetime.fromisoformat(self.start.replace('Z', '+00:00').replace('+00:00', ''))
            if now < start_dt:
                return False
...
class ProofOfScopeSettings(BaseModel):
    ...
    outside_window_acknowledged_at: Optional[str] = None
