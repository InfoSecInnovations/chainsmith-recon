"""
app/scenario_services/ai/rag.py

Simple RAG (Retrieval-Augmented Generation) module.

This module provides a basic RAG implementation with synthetic customer data
that can be leaked via prompt injection. Used by chatbot services to provide
context-aware responses.

The RAG system contains:
- FAQ content (always accessible)
- Synthetic customer data (leakable via injection)
...
FAQ_CONTENT = [
    {
        ...
        "answer": "Our customer service team is available Monday through Friday, 8AM to 8PM EST, and Saturday 9AM to 2PM EST. You can reach us at 1-800-555-BANK.",
    },
    {
        "question": "How do I report a lost or stolen card?",
        "answer": "You can report a lost or stolen card 24/7 by calling 1-800-555-BANK, using our mobile app, or asking me to help you right now. I can lock your card immediately.",
    },
    {
        "question": "What is the overdraft policy?",
        "answer": "We offer overdraft protection with a $35 fee per occurrence, up to 4 times per day. You can opt out of overdraft coverage in your account settings or by visiting a branch.",
    },
    {
...
class SimpleRAG:
    ...
    def __init__(self, customers: list[dict] = None, faqs: list[dict] = None):
        self.customers = customers or SYNTHETIC_CUSTOMERS
        self.faqs = faqs or FAQ_CONTENT
            ...
            if relevant_customers:
                ...
                for c in relevant_customers[:3]:  # Limit exposure
                    context_parts.append(
                        f"- {c['name']} ({c['member_since']} member): {', '.join(c['account_types'])}"
                    )

        return "\n\n".join(context_parts) if context_parts else ""

    def get_customer_context_for_session(self, customer_index: int = 2) -> str:
        """
...
def get_rag_context(query: str) -> str:
    ...
    return get_rag().search_context(query)


def get_session_context() -> str:
    """Get current session customer context."""
    return get_rag().get_customer_context_for_session()
