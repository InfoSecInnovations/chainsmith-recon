...
async def probe_chatbot(
    chat_url: str, message: str, timeout: float = 30.0, extract_headers: bool = True
) -> dict:
    """
...
async def trigger_error(chat_url: str, timeout: float = 15.0) -> dict:
    """
    ...
    async with httpx.AsyncClient(timeout=timeout, verify=False) as client:
        for test in test_cases:
            try:
                response = await client.post(
                    chat_url, json=test["data"], headers={"Content-Type": "application/json"}
                )
                ...
                if response.status_code >= 400:
                    ...
                    error_str = (
                        json.dumps(error_body) if isinstance(error_body, dict) else str(error_body)
                    )
                ...
                results["error_responses"].append({"test_name": test["name"], "error": str(e)})
