...
async def verify_cve(cve_id: str) -> dict:
    ...
    if cve_id in KNOWN_HALLUCINATED_CVES:
        ...
        results["reason"] = f"CVE {cve_id} does not exist in NVD. This appears to be a hallucinated CVE."
    ...
    results["reason"] = f"CVE {cve_id} not found in local database. Recommend manual verification via NVD."
...
async def verify_version_claim(
    software: str,
    claimed_version: str,
    actual_evidence: Optional[str] = None
) -> dict:
    """
    ...
    if actual_evidence:
        ...
        if claimed_version.lower() in actual_evidence.lower():
            ...
            results["reason"] = f"Claimed version {claimed_version} not found in evidence. Evidence shows: {actual_evidence[:100]}"
    else:
