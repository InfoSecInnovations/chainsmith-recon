...
async def internal_redirect(path: str):
    ...
    """
    return RedirectResponse(url=f"https://vpn.fakobanko.local/api/internal/{path}", status_code=302)


@app.get("/internal/{path:path}")
async def internal_redirect_alt(path: str):
    """Alternative internal path - also redirects to VPN."""
    return RedirectResponse(url=f"https://vpn.fakobanko.local/internal/{path}", status_code=302)
